...
```{r data_manhattan}
manhattan <- read_csv("~/R/LectureNotes/lecture-note-class-13-quandoan-cs/manhattan.csv")
```
...
What does this mean? 95 % confident that the true mean is in the confidence
interval
...
What does this mean? We are 95% confident that the proportion of mouse who died
on the treatment is between 69% to 79.7%.
...
```{r practice_2_ci}
ci_mice_lived_treatment <- get_ci(boot_prop2, level = .90)
ci_mice_lived_treatment
```
...
visualize(boot_prop2) +
  shade_ci(ci_mice_lived_treatment)
```


Give an interpretation of your result.
90% confident that the proportion of mice who lived while on the treatment are
between 55.2% to 65.1%.
